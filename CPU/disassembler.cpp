...
// These macros compile on Linux:
#define DISASMTEXT(t) .code = [](auto) { return t; }
#define DISASMFUNC(func) .code = [this](const InstructionBytes &ib) { return this->func(ib); }
// Just in case problems arise after i changed them, these are the versions that compiled under MacOs:
...
Disassembler::Disassembler()
{
    ...
    instructions = {
        ...
        {.numBytes = 2, .cycles = 3, DISASMFUNC(JR_n)},       // 0x18 "JR N"
        ...
        {.numBytes = 2, .cycles = 3, DISASMFUNC(JR_cc_nn)},      // 0x20 "JR NZ,s8"
        ...
        {.numBytes = 2, .cycles = 3, DISASMFUNC(JR_cc_nn)},       // 0x28 "JR Z,s8"
        ...
        {.numBytes = 2, .cycles = 3, DISASMFUNC(JR_cc_nn)},      // 0x30 "JR NC,s8",
        ...
        {.numBytes = 2, .cycles = 3, DISASMFUNC(JR_cc_nn)},       // 0x38 "JR C,s8"
        ...
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB B")},    // 0x90 "SUB B"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB C")},    // 0x91 "SUB C"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB D")},    // 0x92 "SUB D"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB E")},    // 0x93 "SUB E"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB H")},    // 0x94 "SUB H"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB L")},    // 0x95 "SUB L"
        {.numBytes = 1, .cycles = 8, DISASMTEXT("SUB (HL)")}, // 0x96 "SUB (HL)"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SUB A")},    // 0x97 "SUB A"

        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC B")},    // 0x98 "SBC B"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC C")},    // 0x99 "SBC C"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC D")},    // 0x9a "SBC D"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC E")},    // 0x9b "SBC E"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC H")},    // 0x9c "SBC H"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC L")},    // 0x9d "SBC L"
        {.numBytes = 1, .cycles = 8, DISASMTEXT("SBC (HL)")}, // 0x9e "SBC (HL)"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("SBC A")},    // 0x9f "SBC A"

        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND B")},    // 0xa0 "AND B"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND C")},    // 0xa1 "AND C"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND D")},    // 0xa2 "AND D"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND E")},    // 0xa3 "AND E"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND H")},    // 0xa4 "AND H"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND L")},    // 0xa5 "AND L"
        {.numBytes = 1, .cycles = 8, DISASMTEXT("AND (HL)")}, // 0xa6 "AND (HL)"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("AND A")},    // 0xa7 "AND A"

        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR B")},    // 0xa8 "XOR B"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR C")},    // 0xa9 "XOR C"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR D")},    // 0xaa "XOR D"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR E")},    // 0xab "XOR E"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR H")},    // 0xac "XOR H"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR L")},    // 0xad "XOR L"
        {.numBytes = 1, .cycles = 8, DISASMTEXT("XOR (HL)")}, // 0xae "XOR (HL)"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("XOR A")},    // 0xaf "XOR A"

        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR B")},    // 0xb0 "OR B"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR C")},    // 0xb1 "OR C"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR D")},    // 0xb2 "OR D"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR E")},    // 0xb3 "OR E"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR H")},    // 0xb4 "OR H"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR L")},    // 0xb5 "OR L"
        {.numBytes = 1, .cycles = 8, DISASMTEXT("OR (HL)")}, // 0xb6 "OR (HL)"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("OR A")},    // 0xb7 "OR A"

        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP B")},    // 0xb8 "CP B"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP C")},    // 0xb9 "CP C"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP D")},    // 0xba "CP D"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP E")},    // 0xbb "CP E"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP H")},    // 0xbc "CP H"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP L")},    // 0xbd "CP L"
        {.numBytes = 1, .cycles = 8, DISASMTEXT("CP (HL)")}, // 0xbe "CP (HL)"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("CP A")},    // 0xbf "CP A"

        {.numBytes = 1, .cycles = 5, DISASMFUNC(RET_cc)},   // 0xc0 "RET NZ"
        {.numBytes = 1, .cycles = 3, DISASMTEXT("POP BC")}, // 0xc1 "POP BC"
        {.numBytes = 3, .cycles = 4, DISASMFUNC(JP_cc_nn)}, // 0xc2 "JP NZ NN"
        {.numBytes = 3, .cycles = 4, DISASMFUNC(JP_nnnn)},      // 0xc3 "JP NN"
        // {12, &Disassembler::CALL_cc_nn},    // 0xc4 "CALL NZ,NN"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("PUSH BC")}, // 0xc5 "PUSH BC"
        // { 8, &Disassembler::ADD_A_n},       // 0xc6 "ADD A,n"
        // {16, &Disassembler::RST},           // 0xc7 "RST 00h"
        {.numBytes = 1, .cycles = 5, DISASMFUNC(RET_cc)}, // 0xc8 "RET Z"
        // {16, &Disassembler::RET},           // 0xc9 "RET"
        {.numBytes = 3, .cycles = 4, DISASMFUNC(JP_cc_nn)},      // 0xca "JP Z,**"
        ...
        {.numBytes = 1, .cycles = 5, DISASMFUNC(RET_cc)},   // 0xd0 "RET NC"
        {.numBytes = 1, .cycles = 3, DISASMTEXT("POP DE")}, // 0xd1 "POP DE"
        {.numBytes = 3, .cycles = 4, DISASMFUNC(JP_cc_nn)},      // 0xd2 "JP NC,NN"
        // { 0, &Disassembler::invalid_opcode},// 0xd3 "OUT (N),A"
        // {12, &Disassembler::CALL_cc_nn},    // 0xd4 "CALL NC,NN"
        {.numBytes = 1, .cycles = 4, DISASMTEXT("PUSH DE")}, // 0xd5 "PUSH DE"
        // { 8, &Disassembler::SUB_n},         // 0xd6 "SUB N"
        // {16, &Disassembler::RST},           // 0xd7 "RST 10h"
        {.numBytes = 1, .cycles = 5, DISASMFUNC(RET_cc)}, // 0xd8 "RET C"
        // {16, &Disassembler::RETI},          // 0xd9 "EXX"
        {.numBytes = 3, .cycles = 4, DISASMFUNC(JP_cc_nn)},      // 0xda "JP C,NN"
